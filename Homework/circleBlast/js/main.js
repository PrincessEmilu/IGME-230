...
function fireBullet(e){
    ...
    //Loop control variables for creating bullets; changes if tripleshot enabled
    let startingI = 0;
    let endingI = 1;
    if (tripleShot){
        startingI = -1;
    ...
    //Loop and create bullets with displacements based on i
    for(let i = startingI; i < endingI; i++)
    {
        let b = new Bullet(0xFFFFFF, ship.x + 10 * i, ship.y);
        bullets.push(b);
        gameScene.addChild(b);
    }

    //Play shoot sound last, only once
    shootSound.play();
}
...
function end(){
    ...
    //clear out level
    circles.forEach(c=>gameScene.removeChild(c)); //COncise arrow function, no brackets, no return
    circles = [];
...
function gameLoop(){
    ...
    let amt = 6 * dt; //at 60 FPS move abount 10% of distance per update

    //lerp the x and y values with lerp()
    let newX = lerp(ship.x, mousePosition.x, amt);
    let newY = lerp(ship.y, mousePosition.y, amt);

    //keep ship on the screen with clamp()
    let w2 = ship.width/2;
